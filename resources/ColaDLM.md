![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/d4babf43d44fbf27b74f48c93d8a955c4c0fa4d033ff92a0b1dae8bb4bcf6b21.jpg)
...
$$
p _ {\psi} (z _ {0}) = p _ {\psi} (z _ {0} ^ {(1)}) \prod_ {b = 2} ^ {B} p _ {\psi} (z _ {0} ^ {(b)} \mid z _ {0} ^ {(<   b)}). \tag {3.3}
$$
...
$$
\log p (x) \geq \mathbb {E} _ {q _ {\phi} (z _ {0} | x)} [ \log p _ {\theta} (x \mid z _ {0}) + \log p _ {\psi} (z _ {0}) - \log q _ {\phi} (z _ {0} \mid x) ] =: \mathcal {L} _ {\mathrm{ELBO}} (x). \tag {3.4}
$$
...
$$
\mathbb {E} _ {p _ {\mathrm{data}} (x)} [ \mathcal {L} _ {\mathrm{ELBO}} (x) ] = \mathbb {E} _ {q (x, z _ {0})} [ \log p _ {\theta} (x \mid z _ {0}) ] - I _ {q} (X; Z _ {0}) - \mathrm{KL} (\bar {q} _ {\phi} (z _ {0}) \| p _ {\psi} (z _ {0})), \tag {3.5}
$$

where $q ( x , z _ { 0 } ) = p _ { \mathrm { d a t a } } ( x ) q _ { \phi } ( z _ { 0 } \mid x )$ . This decomposition shows that Cola DLM separates text modeling into conditional reconstruction, information compression, and prior matching. 

When the encoder and decoder are fixed, prior learning reduces to 

$$
\max _ {\psi} \mathbb {E} _ {z _ {0} \sim \bar {q} _ {\phi}} [ \log p _ {\psi} (z _ {0}) ] \quad \Longleftrightarrow \quad \min _ {\psi} \mathrm{KL} (\bar {q} _ {\phi} (z _ {0}) \| p _ {\psi} (z _ {0})). \tag {3.6}
$$
...
2: sample $z_{0}^{(k)} \sim q_{\phi}(z_{0} \mid x)$ 3: $w^{(k)} \leftarrow \log p_{\theta}(x \mid z_{0}^{(k)}) + \log p_{\psi}(z_{0}^{(k)}) - \log q_{\phi}(z_{0}^{(k)} \mid x)$ 4: end for
5: output $\log \widehat{p}_{\mathrm{ELBO}}(x) = \frac{1}{K} \sum_{k=1}^{K} w^{(k)}$ or $\log \widehat{p}_{\mathrm{IWAE}}(x) = \log \frac{1}{K} \sum_{k=1}^{K} \exp(w^{(k)})$ according to Eq. (3.12) 
...
1: estimate $\log\widehat{p}(x_{\mathrm{pre}}, x_{\mathrm{res}})$ using Algorithm 1
2: estimate $\log\widehat{p}(x_{\mathrm{pre}})$ using Algorithm 1
3: return $\log\widehat{p}(x_{\mathrm{res}} \mid x_{\mathrm{pre}})$ $=\log\widehat{p}(x_{\mathrm{pre}}, x_{\mathrm{res}})-\log\widehat{p}(x_{\mathrm{pre}})$ according to Eq. (3.14) 

# 3.1.2 Probability Estimation for Cola DLM

Unconditional probability estimation. At evaluation time, we approximate log p(x). For samples $z _ { 0 } ^ { ( k ) }$ ∼ $q _ { \phi } ( z _ { 0 } \mid x )$ , define the importance weight 

$$
\log w ^ {(k)} = \log p _ {\theta} (x \mid z _ {0} ^ {(k)}) + \log p _ {\psi} (z _ {0} ^ {(k)}) - \log q _ {\phi} (z _ {0} ^ {(k)} \mid x). \tag {3.8}
$$
...
$$
\frac {\mathrm{d}}{\mathrm{d} t} \left[ \begin{array}{c} z _ {t} \\ \ell_ {t} \end{array} \right] = \left[ \begin{array}{c} \boldsymbol {v} _ {\psi} (z _ {t}, t) \\ \nabla \cdot \boldsymbol {v} _ {\psi} (z _ {t}, t) \end{array} \right], \quad \left[ \begin{array}{c} z _ {0} \\ \ell_ {0} \end{array} \right] = \left[ \begin{array}{c} z _ {0} ^ {(k)} \\ 0 \end{array} \right], \tag {3.9}
$$

from t = 0 to t = 1, yielding $( z _ { 1 } ^ { ( k ) } , \ell _ { 1 } ^ { ( k ) } )$ . We then obtain 

$$
\log p _ {\psi} (z _ {0} ^ {(k)}) = \log p _ {1} (z _ {1} ^ {(k)}) + \ell_ {1} ^ {(k)}, \tag {3.10}
$$
...
$$
\nabla \cdot \boldsymbol {v} _ {\psi} (z _ {t}, t) = \mathrm{Tr} \bigg (\frac {\partial \boldsymbol {v} _ {\psi} (z _ {t} , t)}{\partial z _ {t}} \bigg) \approx \boldsymbol {\epsilon} ^ {\top} \frac {\partial \boldsymbol {v} _ {\psi} (z _ {t} , t)}{\partial z _ {t}} \boldsymbol {\epsilon}, \qquad \boldsymbol {\epsilon} \sim \mathcal {N} (0, I), \tag {3.11}
$$

where the same ϵ is fixed within one ODE solve. 

This gives two standard estimators, namely the ELBO-style and IWAE-style estimators: 

$$
\log \widehat {p} _ {\mathrm{ELBO}, K} (x) = \frac {1}{K} \sum_ {k = 1} ^ {K} \log w ^ {(k)}, \quad \log \widehat {p} _ {\mathrm{fWAE}, K} (x) = \log \left(\frac {1}{K} \sum_ {k = 1} ^ {K} e ^ {\log w ^ {(k)}}\right). \tag {3.12}
$$
...
$$
\log p (x _ {\text { res }} \mid x _ {\text { pre }}) = \log p (x _ {\text { pre }}, x _ {\text { res }}) - \log p (x _ {\text { pre }}). \tag {3.13}
$$
...
$$
\log \widehat {p} (x _ {\text { res }} \mid x _ {\text { pre }}) = \log \widehat {p} (x _ {\text { pre }}, x _ {\text { res }}) - \log \widehat {p} (x _ {\text { pre }}). \tag {3.14}
$$
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/020c588e48d714f3bd7a61726e549aaa0e3934aa254df23cbecaeef2f86b0fd8.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/e4cb6045810b29a2a1ac283cb13e7e8000decbb3823138f6534ce52ac5e028c5.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/90b409b9888cad649ea035d0f90c96e731ec7dcb2760d6ab54d5cf1ca574b8e8.jpg)
...
$$
z _ {0} \sim q _ {\phi} (z _ {0} \mid x), \quad \hat {x} \sim p _ {\theta} (x \mid z _ {0}). \tag {3.15}
$$
...
$$
\mathcal {L} _ {\mathrm{VAE}} = - \mathbb {E} _ {q _ {\phi} (z _ {0} | x)} \log p _ {\theta} (x \mid z _ {0}) + \beta \operatorname{KL} (q _ {\phi} (z _ {0} \mid x) \| p _ {\text {base}} (z _ {0})) + \lambda_ {\text {mask}} \mathcal {L} _ {\text {mask}}. \tag {3.16}
$$
...
$$
\mathcal {V} _ {b} = \left\{\mathrm{sg} (z _ {0} ^ {(<   b)}), z _ {t} ^ {(b)} \right\}, \tag {3.17}
$$
...
$$
\begin{array}{l} \mathcal {L} _ {\text { stage2 }} = \lambda_ {\text { VAE }} \left(- \mathbb {E} _ {q _ {\phi} (z _ {0} | x)} \log p _ {\theta} (x \mid z _ {0}) + \beta \mathbb {E} _ {q _ {\phi} (z _ {0} | x)} \log q _ {\phi} (z _ {0} \mid x) + \lambda_ {\text { mask }} \mathcal {L} _ {\text { mask }}\right) \\ + \lambda_ {\mathrm{fm}} \mathcal {L} _ {\mathrm{FM}} + \lambda_ {\mathrm{ref}} \mathbb {E} _ {p _ {\mathrm{data}} (x)} \mathrm{KL} \left(q _ {\phi} \left(z _ {0} \mid x\right) \| q _ {\phi_ {\mathrm{ref}}} \left(z _ {0} \mid x\right)\right). \\ \end{array}
$$
...
$$
z ^ {\mathrm{pre}} \sim q _ {\phi} (z ^ {\mathrm{pre}} \mid x ^ {\mathrm{pre}}). \tag {3.19}
$$
...
$$
\hat {z} _ {0} ^ {(b)} = \Phi_ {0 \leftarrow 1} ^ {\psi} \Bigl (\epsilon^ {(b)}; z ^ {\mathrm{pre}}, \hat {z} _ {0} ^ {(<   b)} \Bigr), \quad \epsilon^ {(b)} \sim \mathcal {N} (0, I). \tag {3.20}
$$

Finally, the decoder outputs the text response conditioned on the prefix and the generated latent blocks: 

$$
\hat {x} ^ {\text { res }} \sim p _ {\theta} \big (x ^ {\text { res }} \mid z ^ {\text { pre }}, \hat {z} _ {0} ^ {(1: B)} \big). \tag {3.21}
$$
...
For a unified comparison, let $\tau = ( S _ { t } ) _ { t \in \mathcal { T } }$ be a stochastic process on state space S, with initial distribution $\mu _ { \Theta }$ , transition kernel $K _ { t } ^ { \Theta }$ , and emission mechanism $e _ { \Theta } ( x \mid \tau )$ . A process-based generative model can be written as 

$$
p _ {\Theta} (x) = \int e _ {\Theta} (x \mid \tau) P _ {\Theta} (d \tau), \quad P _ {\Theta} (d \tau) = \mu_ {\Theta} (d s _ {0}) \prod_ {t > 0} K _ {t} ^ {\Theta} (d s _ {t} \mid s _ {<   t}). \tag {3.22}
$$
...
$$
p _ {\mathrm{AR}} (x) = \prod_ {i = 1} ^ {L} p _ {\eta} (x _ {i} \mid x _ {<   i}). \tag {3.23}
$$

For $\mathrm { L L a D A }$ , the path is a discrete corruption–recovery trajectory, whose objective is observation reconstruction in a discrete state space: 

$$
q (s _ {1: T} \mid x) = q _ {1} (s _ {1} \mid x) \prod_ {t = 2} ^ {T} q _ {t} (s _ {t} \mid s _ {t - 1}), \quad p _ {\theta} (s _ {0: T}) = p (s _ {T}) \prod_ {t = 1} ^ {T} p _ {\theta} (s _ {t - 1} \mid s _ {t}). \tag {3.24}
$$
...
<table><tr><td>Method</td><td>State Space</td><td>Path Role</td><td>Generative Factorization</td><td>Where Continuity Appears</td><td>Explicit Latent</td></tr><tr><td>AR</td><td>Prefix Tokens</td><td>Direct Generation Path</td><td><eq>\prod_{i} p(x_{i} \mid x_{&lt;i})</eq></td><td>None</td><td>✕</td></tr><tr><td>LLaDA</td><td>Discrete Masked Sequences</td><td>Discrete Observation-Recovery Path</td><td><eq>p(s_{T}) \prod_{t} p_{\theta}(s_{t-1} \mid s_{t})</eq></td><td>Discrete Token Space</td><td>✕</td></tr><tr><td>Plaid</td><td>Continuous Token-Aligned Representations</td><td>Continuous Observation-Recovery Path</td><td><eq>p(h_{T}) \prod_{t} p_{\theta}(h_{t-1} \mid h_{t})</eq></td><td>Continuous Token Space</td><td>✕</td></tr><tr><td>Cola DLM</td><td>Compressed Latent Sequences</td><td>Prior-Transport Path</td><td><eq>\int p_{\theta}(x \mid z_{0}) p_{\psi}(z_{0}) dz_{0}</eq></td><td>Latent Space</td><td>√</td></tr></table>
...
$$
q (h _ {1: T} \mid h _ {0}) = q _ {1} (h _ {1} \mid h _ {0}) \prod_ {t = 2} ^ {T} q _ {t} (h _ {t} \mid h _ {t - 1}), \quad p _ {\theta} (h _ {0: T}) = p (h _ {T}) \prod_ {t = 1} ^ {T} p _ {\theta} (h _ {t - 1} \mid h _ {t}). \tag {3.25}
$$
...
$$
z _ {1} \sim p _ {1}, \quad z _ {0} = \Phi_ {0 \leftarrow 1} ^ {\psi} (z _ {1}), \quad x \sim p _ {\theta} (x \mid z _ {0}), \tag {3.26}
$$
...
The reason for using a continuous path is not that continuous modeling is inherently superior, but that it naturally captures the geometry of the latent distribution. In Cola DLM, continuity appears in $p _ { \psi } ( z _ { 0 } )$ , rather than in an observation-recovery trajectory: 

$$
\frac {d z _ {t}}{d t} = v _ {\psi} (z _ {t}, t), \quad p _ {\psi} = (\Phi_ {0 \leftarrow 1} ^ {\psi}) _ {\sharp} p _ {1}. \tag {3.27}
$$
...
$$
\mathbb {E} _ {p _ {\mathrm{data}} (x)} [ \mathcal {L} _ {\mathrm{ELBO}} (x) ] = \mathbb {E} _ {q (x, z _ {0})} [ \log p _ {\theta} (x \mid z _ {0}) ] - I _ {q} (X; Z _ {0}) - \mathrm{KL} (\bar {q} _ {\phi} (z _ {0}) \| p _ {\psi} (z _ {0})), \tag {3.28}
$$

shows that $z _ { 0 }$ is not merely a continuous surrogate for discrete text, but an explicit marginalized intermediate variable: global semantics are compressed into z0, while local token realization is delegated to the decoder. 

# 3.3.2 Theoretical Advantages of Cola DLM

A unified criterion. Let the lower bound of the approximation error for a model family M be 

$$
\mathcal {E} (\mathcal {M}) := \inf _ {p \in \mathcal {M}} \mathrm{KL} (p _ {\text { data }} (x) \| p (x)). \tag {3.29}
$$
...
$$
G _ {\text { infer }} ^ {\text { ColaDLM }} := \mathbb {E} _ {p _ {\text { data }} (x)} \mathrm{KL} \big (q _ {\phi} (z _ {0} \mid x) \| p _ {\theta , \psi} (z _ {0} \mid x) \big). \tag {3.30}
$$

Its total statistical burden is therefore 

$$
R _ {\text { ColaDLM }} = \mathcal {E} (\mathcal {M} _ {\text { ColaDLM }}) + \inf _ {\phi , \theta , \psi} G _ {\text { infer }} ^ {\text { ColaDLM }}. \tag {3.31}
$$
...
$$
\mathcal {C o l a D L M} \succ \mathrm{AR} \quad \Longleftrightarrow \quad R _ {\text {ColaDLM}} <   \mathcal {E} (\mathcal {M} _ {\mathrm{AR}}). \tag {3.32}
$$
...
$$
D (R) := \inf_ {\substack {q \left(z _ {0} \mid x\right): \\ I _ {q} \left(X; Z _ {0}\right) \leq R}} \inf_ {p _ {\theta} \left(x \mid z _ {0}\right)} \mathbb {E} _ {q \left(x, z _ {0}\right)} \left[ - \log p _ {\theta} \left(x \mid z _ {0}\right) \right]. \tag{3.33}
$$
...
$$
p _ {\mathrm{data}} (x) = \int p ^ {\star} (x \mid g)   p ^ {\star} (g)   d g, \qquad H (X \mid G) \ll H (X), \qquad \dim (G) \ll \dim (E (X)), \tag {3.34}
$$
...
$$
\mathcal {C} o l a \mathrm{DLMisadvantageous} \Longleftrightarrow \left[ D (R) \text {is already small at low} R \right]
$$

$$
\wedge \left[ \mathcal {E} (\mathcal {M} _ {\text { ColaDLM }}) \text {   decreases } \right] \tag {3.35}
$$

$$
\wedge \left[ G _ {\text { infer }} ^ {\text { ColaDLM }} \text {   is   controllable } \right].
$$
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/0e100b944c822548761fdf82e0f82c03624991e941088e3e756600a2568e6d14.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/e403234184604228b406861bbaa7476ebd18833a011cad2ddaae86aef0e4fe2b.jpg)
...
Fixed vs. Evolving Latent Space. As shown in Figure 3, this section studies whether the latent space should evolve jointly with DiT during training. Under the same compute budget, we compare five strategies: fixing a pretrained VAE (Fix VAE); initializing the VAE from pretrained weights and jointly training it with DiT using a VAE learning rate equal to that of DiT or scaled to 0.01× (Joint DiT x1 / Joint DiT x0.01); jointly training both VAE and DiT from random initialization with the same learning rate (All Scratch x1); and an interval-based strategy (Interval), where each 5k-step cycle consists of 2k steps of joint training followed by 3k steps with the VAE frozen. The overall results suggest that the latent space should neither remain fully fixed nor be jointly optimized from scratch without constraint. Instead, the most effective strategy is to let it evolve together with DiT on top of a stable initialization. 

![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/13dfa2e8d81170f41e351df988395a9034d165f03ab315257c7179d2179f1ebd.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/848abaa5d550c19b8bc9d5a666c9259ce85b3b095d2076f0233dbdc38b66dc67.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/11ba33656130d484c5ec3d3fbc50621ad2e2cb47d55c57c35c07c50de82277c3.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/55d0df21af93b87f12cec00917997f0759da2de95d782017788650cce41b2a01.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/442b85916e30b8305d8385f32485f14f43135c7dce1d5deeb6c777f9f6bcc905.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/45f40e895a71f3531f4ddbd83947c62d0279e9dc797013b093e962bf22ac37b8.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/a5b258ac159d425de176943f54cd1660beaaa1a92c484d5dd45229f4c8e5af0c.jpg)
...
Obs. ❶ Increasing the latent dimension improves the overall semantic capacity under the same compute budget. As shown in Table 2, the average score increases from 8.7 at d = 16 to 11.3 at $d = 6 4 .$ and further to 11.8 at d = 128. The improvement is most evident on MMLU and SIQA. Although LAMBADA peaks at d = 64, the overall trend still suggests that a larger latent space carries stronger semantic capacity under the 

![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/c4307fa67e506f5dff384113aa9af600a00e4d1761a0314179bf88a9c6ffb6d6.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/9edcd5e860f37611109973f8f8dd3a354abfe5a78f6774ea7aa4d21b13a0eb16.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/a28dabc356791425b8afc868e7c16479b4a7209787e50810bb05814af048572f.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/255bf9997e0ee1dc1c12dcdd0a342e391e759cf058aaf755ac393d1b5a038feb.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/72f99dff8bdccd991136a271f893c15cef9b2f6a1dbc42c4b145790d390cd6e0.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/681b50251f6a9f68425a3b03772781e03ed480a608bb7cff8969fb6db24e35a7.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/e101e7af77885cb1dc79d552cdfd8ee9db1f5907a29946a7582f541afe87a164.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/5467ec3e2e54b790da01d75397314d4e8007b815e001dfa490426f8a735257dd.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/6f61d5b8a07b8f06ab14bb9fe5bcb477a1d79669a50391c307b8bb9c8c279d3c.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/0aaea6148d3fe63ba05d5686c5403db3d5ad72586c6e150ca361d789b0547efe.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/cdab8e79043e5867f5b49e319bf0c27fd5ba37ded5946b63266c0d136b82e3b7.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/a93f31ae7fc7db762de7ac193c95eb6d2cb43a1defa1efe7f4bc4eb897562fe9.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/eaab30211e2add9976ed6342216be1a51b394a1d89a5ce80f2ee9343b4da3a5b.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/5daecc8cccbf86a0c0fe35c559a74f09353be3cc3451ff13dafe0c377d229561.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/00659b43d444cdfb8301e04382781dc13884f4a0d33c370e64e0bb62c093f0d0.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/e0c1642da5374119045b3c9c40b80de7bd5fcdbd7b31479c7e1b1929781896b4.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/ec63d2fc14c96d6e6b25117c86d4f36ccfdecce96850cd916f1788893bb12852.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/9d984db21273a97ecddb740565532c10adae6d34e559fd7b72cad767efad7395.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/431f6a8e6f305ee05c9e43b5144deadf9ac95fb196b4d2dee70ed8aebd1d911c.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/155d8216a5bc65c1c902d94bdf3bb0afc9babc73e2f49e21c195a9df9d0915c7.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/e1d0b0a6d3fc369894880750587e5a460bcfcd5f6a4152a7fe579ad97d626889.jpg)
...
Obs. ❷ Excessive guidance leads to clear degradation. After the moderate optimum region, all task curves begin to decline as CFG becomes larger. The drop is especially pronounced beyond CFG ≈ 10, and becomes severe at very large values such as 20 and 60. This shows that overly strong guidance distorts the denoising trajectory rather than improving it. 


MMLU


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/560ac633e4eaafe2ab0f2eebe4138ebda747b3fd6d794fb9cfe2451184678974.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/24fe064b4ee31e03c544edf4a1b84e8858c75c74d50629fa8d26a33fff4afbd4.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/15a6e0b83ec1414a430d369f79e9b3357a543cb07ccd686d24fe763cbb258728.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/ea8420d6dbffc590e0805e3cf8bd1b2e8510a4f89485f47169710cbac66643d5.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/08c354d6627ceeb4edf129baf8c905785c288ba2d66b00bbc5c110383dd59205.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/dd2aebbbcd33a0984d9df32f69c97f4c72518ade494bf6ad0fd3a2125d59a486.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/e9fa615f60bb1f9278a89e3558e0476fa7a973bb1f4a9e536ae8b21208f0c1e2.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/ad634739d2742dca8f9554db2a7b153a96ef21ec130e92d7b0211c2c567d0f23.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/7489141219a2a3c349c4951788915a372b295f6a711b56720e102ba339a6dee5.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/cc598267c614c0d79abb9a54f9ca408bcb1923d69ce93fe116baf85f15fe80ba.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/a3ec8467490c21a6dd266a00d1c4bae20b0d8cc036f0fdfc55c761ec7f313629.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/23289564b453343e7f9a513b5c8543020880d52130b9099b7c74d94f676c4781.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/de0e87ee27a2c788d5025b8e1a2d99e5f3283ca1cb94a3aecdc12a0c0290cbbe.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/9fa15679015471693b533d9e948e49e601f6530efc2bc7e85b6240b44564ddca.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/2c33728f93fae2b1766055bb4ef3a178bb6fa3d2021e24eb47f42513186c5282.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/c1fdea313af4fe0e1c14a9bb76c8d99c21d318f6b990558e833548c7d9b2bd4a.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/9293aa7a53471dffd95411cd54f554ec02904daac9f551b58baeaa9512bc8386.jpg)
...
$$
p (x ^ {\text { res }} \mid c) = \int p _ {\theta} (x ^ {\text { res }} \mid z, c)   p _ {\psi} (z \mid c)   \mathrm{d} z, \tag {5.1}
$$

while the practically accessible quantity is the local score 

$$
\mathcal {S} _ {\mathrm{resp}} (x) = \mathbb {E} _ {q _ {\phi} (z | x, c)} \Big [ \log p _ {\theta} (x ^ {\mathrm{res}} \mid z, c) + \log p _ {\psi} (z \mid c) - \log q _ {\phi} (z \mid x, c) \Big ]. \tag {5.2}
$$
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/59361fbc4253fb0d111251f9f76e0b8940d849ed2b653dbedb5339f1d8c885d7.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/4435af1c5ec2dc99104d6f766ddc9e0a354ffde2b7db925e895a5357353226ef.jpg)
...
$$
z _ {0} ^ {\mathrm{text}} \sim q _ {\phi_ {\mathrm{text}}} (z \mid x ^ {\mathrm{text}}), \qquad z _ {0} ^ {\mathrm{img}} \sim q _ {\phi_ {\mathrm{img}}} (z \mid x ^ {\mathrm{img}}).
$$

We then define a joint latent state 

$$
\tilde {z} _ {0} = \big (z _ {0} ^ {\mathrm{text}}, z _ {0} ^ {\mathrm{img}} \big),
$$

and model the unified generative process as 

$$
p (x ^ {\mathrm{text}}, x ^ {\mathrm{img}}, \tilde {z} _ {0}) = p _ {\theta} \left(x ^ {\mathrm{text}}, x ^ {\mathrm{img}} \mid \tilde {z} _ {0}\right) p _ {\psi} (\tilde {z} _ {0}).
$$
...
The summer night was quiet and peaceful. Stars twinkled in the deep blue sky. 
...
I still remember the happiest days of my childhood. Every summer, my friends and 

# Generation:

I would go out on the beach and look out at the ocean, enjoying our time together towards the left of the garden... 

# Image x Text to Text

![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/a9735af333895123ac1acf1b720b39ef6cf4a4d35a11ece0356e78581ae0a822.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/08058f3db8950b9f8a17df97674c8f4cbe379ee7a5e27dc9746942635ec49461.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/63afae0eef82fb7ff7b98d84af7afe6fd57fc3e7208615655de1c1a3a1b86211.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/400381b6c14e66e8678b80d56bd31e35ea596bb721ea3de9f1beac6f5baf3a4d.jpg)
...
Prompt: 1. What is shown in this image? 2. Describe the content of this image in detail. 

(a) City skyscrapers rising from the water, with many buildings and roads on the sea. 

(b) A person using a pen to write on a piece of paper. 

(c) Some people sitting around a long table in a conference room. 
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/5c2bb2c9cff8660c3c5b8c0355c688f26b9c6e89936b862d70578f520cab016d.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/6a6bb8b5358321973b61f9e888c308386efcc8aeddeb0b0e48389c93ffaae208.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/9b0cfe04be2222000fc81ed05f39735d3e9100925045b9b17ed5a90fae8bf787.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/0431c7c9e903ff7711fad2e93d0012ba7af579658b089e45f255c84893217732.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/7c8971706f3d617af622d57fd33b0647eb84f06f99133581423107f35246726f.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/2a2e5871b21cbc7292f4b74ae95637feedbbcc2e1df59a1b0a13bfe05b5c8edb.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/a9d3e37afddc0a4d908ac0ad6cea400c9e49359b70ff30e75023eae28bec0665.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/62695998909eaca628dfada91f4fe049251ab30e8edd230e36ef66cff195284e.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/7b6e374f03ec94bfd45e69ba7a22694b90b25fe3f21d54fdfbca5cff8aca2995.jpg)
...
$$
\mathbb {E} [ \mathcal {L} _ {\mathrm{ELBO}} ] = \mathbb {E} _ {q} \big [ \log p _ {\theta} (x ^ {\mathrm{text}}, x ^ {\mathrm{img}} \mid \tilde {z} _ {0}) \big ] - I \Big ((X ^ {\mathrm{text}}, X ^ {\mathrm{img}}); \tilde {Z} _ {0} \Big) - \mathrm{KL} (\bar {q} (\tilde {z} _ {0}) \parallel p _ {\psi} (\tilde {z} _ {0})) ,
$$
...
$$
o _ {t} \sim P _ {\mathcal {E}} (\cdot \mid \omega_ {t}), \tag {8.2}
$$

$$
h _ {t} = U _ {\theta} (h _ {t - 1}, o _ {t}), \tag {8.3}
$$
...
$$
\xi_ {t} \sim \mathcal {F} (\cdot \mid \omega_ {t}, o _ {t}, a _ {t}), \tag {8.5}
$$

$$
\omega_ {t + 1} \sim \mathcal {T} (\cdot \mid \omega_ {t}, o _ {t}, a _ {t}, \xi_ {t}), \tag {8.6}
$$

$$
\ell_ {t} = \mathcal {G} (\omega_ {t}, o _ {t}, a _ {t}, \xi_ {t}). \tag {8.7}
$$
...
$$
p _ {\mathrm{AR}} (x) = \prod_ {t = 1} ^ {n} p _ {\theta} (x _ {t} \mid x _ {<   t}). \tag {8.9}
$$
...
$$
q (x, z _ {0}) := p _ {\text { data }} (x) q _ {\phi} (z _ {0} \mid x), \tag {8.11}
$$

then 

$$
\mathbb {E} _ {p _ {\mathrm{data}} (x)} \left[ \mathcal {L} _ {\mathrm{ELBO}} (x) \right] = \mathbb {E} _ {q (x, z _ {0})} \left[ \log p _ {\theta} (x \mid z _ {0}) \right] - I _ {q} (X; Z _ {0}) - \mathrm{KL} \left(\bar {q} _ {\phi} (z _ {0}) \| p _ {\psi} (z _ {0})\right), \tag {8.12}
$$
...
$$
- \mathcal {L} _ {\mathrm{ELBO}} (x) = - \log p _ {\theta , \psi} (x) + \mathrm{KL} \big (q _ {\phi} (z _ {0} \mid x) \| p _ {\theta , \psi} (z _ {0} \mid x) \big). \tag {8.13}
$$
...
$$
o _ {t} = \big (o _ {t} ^ {(1)}, o _ {t} ^ {(2)}, \dots , o _ {t} ^ {(M)} \big), \qquad o _ {t} ^ {(m)} \in \mathcal {O} ^ {(m)}, \tag {8.14}
$$
...
$$
\xi_ {t}, \omega_ {t + 1} \sim p \big (\xi_ {t}, \omega_ {t + 1} \mid z _ {t}, a _ {t} \big). \tag {8.16}
$$

If the true environmental dynamics satisfy 

$$
p (\xi_ {t}, \omega_ {t + 1} \mid o _ {t}, a _ {t}) \neq \prod_ {m = 1} ^ {M} p _ {m} \big (\xi_ {t} ^ {(m)}, \omega_ {t + 1} ^ {(m)} \mid o _ {t} ^ {(m)}, a _ {t} ^ {(m)} \big), \tag {8.17}
$$
...
$$
z ^ {\mathrm{text}} \sim q _ {\phi} (z \mid x ^ {\mathrm{text}}), \qquad x ^ {\mathrm{text}} \sim p _ {\eta} (x \mid z ^ {\mathrm{text}}), \tag {8.18}
$$
...
$$
\tilde {z} _ {t} = \Psi \left(z _ {t} ^ {\text { text }}, z _ {t} ^ {\text { img }}, z _ {t} ^ {\text { vid }}, \dots\right), \tag {8.19}
$$
...
$$
q _ {\phi} (z _ {0} \mid x) \quad \text { encoder   (approximate   posterior) }, \tag {A.1}
$$

$$
p _ {\theta} (x \mid z _ {0}) \quad \text { decoder   (conditional   text   generator) }, \tag {A.2}
$$

$$
p _ {\psi} (z _ {0}) \quad \text { latent   prior   (defined   by   a   continuous   flow) }. \tag {A.3}
$$
...
Assumption A.1 (Regularity of the CNF vector field). For each parameter ψ, the vector field $\mathbf { } v _ { \psi } ( z , t )$ is locally Lipschitz in z, measurable in t, and satisfies a linear-growth condition ensuring the existence and uniqueness of global ODE solutions. In addition, $\nabla \cdot \boldsymbol { v } _ { \psi } ( z , t )$ is integrable along the trajectories of interest. Then, for any $0 \leq s \leq t \leq 1$ , the flow map $\Phi _ { t  s } ^ { \psi }$ exists, is unique, and is invertible. 

Let the base distribution be the standard Gaussian 

$$
z _ {1} \sim p _ {1} (z _ {1}) = \mathcal {N} (0, I). \tag {A.4}
$$
...
$$
z _ {t} = \Phi_ {t \leftarrow s} ^ {\psi} (z _ {s}). \tag {A.6}
$$
...
$$
z _ {1} \sim p _ {1} (z _ {1}), \quad z _ {0} = \Phi_ {0 \leftarrow 1} ^ {\psi} (z _ {1}), \quad x \sim p _ {\theta} (x \mid z _ {0}). \tag {A.9}
$$
...
$$
p (x) = \int p _ {1} (z _ {1})   p _ {\theta} \big (x \mid \Phi_ {0 \leftarrow 1} ^ {\psi} (z _ {1}) \big)   \mathrm{d} z _ {1}. \tag {A.11}
$$
...
Let $Z _ { t }$ denote the random variable induced by the ODE at time t, with density $p _ { t }$ . Under Assumption $\mathrm { A . 1 }$ , $p _ { t }$ satisfies the continuity equation 

$$
\partial_ {t} p _ {t} (z) + \nabla \cdot \left(p _ {t} (z) \pmb {\nu} _ {\psi} (z, t)\right) = 0.
$$

Along a characteristic trajectory $t \mapsto z _ { t }$ , the instantaneous change-of-variables formula gives 

$$
\frac {\mathrm{d}}{\mathrm{d} t} \log p _ {t} (z _ {t}) = - \nabla \cdot \boldsymbol {v} _ {\psi} (z _ {t}, t). \tag {A.12}
$$

Integrating from 0 to 1, and noting that $p _ { 0 } = p _ { \psi }$ , yields 

$$
\log p _ {\psi} (z _ {0}) = \log p _ {1} (z _ {1}) + \int_ {0} ^ {1} \nabla \cdot \boldsymbol {v} _ {\psi} (z _ {t}, t)   \mathrm{d} t, \quad z _ {1} = \Phi_ {1 \leftarrow 0} ^ {\psi} (z _ {0}). \tag {A.13}
$$
...
$$
K _ {t \leftarrow s} ^ {\psi} (\mathrm{d} z _ {t} \mid z _ {s}) := \delta_ {\Phi_ {t \leftarrow s} ^ {\psi} (z _ {s})} (\mathrm{d} z _ {t}). \tag {A.14}
$$
...
$$
\log p (x) = \log \int q _ {\phi} \left(z _ {0} \mid x\right) \frac {p _ {\theta} \left(x \mid z _ {0}\right) p _ {\psi} \left(z _ {0}\right)}{q _ {\phi} \left(z _ {0} \mid x\right)} \mathrm{d} z _ {0} \tag {A.15}
$$

$$
\geq \mathbb {E} _ {q _ {\phi} (z _ {0} | x)} \left[ \log p _ {\theta} (x \mid z _ {0}) + \log p _ {\psi} (z _ {0}) - \log q _ {\phi} (z _ {0} \mid x) \right]. \tag {A.16}
$$

Define the outer ELBO as 

$$
\mathcal {L} _ {\mathrm{ELBO}} (x) := \mathbb {E} _ {q _ {\phi} (z _ {0} | x)} \left[ \log p _ {\theta} (x \mid z _ {0}) + \log p _ {\psi} (z _ {0}) - \log q _ {\phi} (z _ {0} \mid x) \right]. \tag {A.17}
$$

Then 

$$
\log p (x) \geq \mathcal {L} _ {\mathrm{ELBO}} (x). \tag {A.18}
$$

More importantly, the exact decomposition between the true log-likelihood and the ELBO is 

$$
\log p (x) = \mathcal {L} _ {\mathrm{ELBO}} (x) + \mathrm{KL} \big (q _ {\phi} (z _ {0} \mid x) \| p (z _ {0} \mid x) \big), \tag {A.19}
$$

where the true posterior is 

$$
p (z _ {0} \mid x) = \frac {p _ {\theta} (x \mid z _ {0}) p _ {\psi} (z _ {0})}{p (x)}. \tag {A.20}
$$
...
$$
\mathcal {L} _ {\mathrm{ELBO}} (x) = \mathbb {E} _ {q _ {\phi} (z _ {0} | x)} \left[ \log p _ {\theta} (x \mid z _ {0}) + \log p _ {1} (z _ {1}) + \int_ {0} ^ {1} \nabla \cdot \boldsymbol {v} _ {\psi} (z _ {t}, t) \mathrm{d} t \right. \tag {A.21}
$$

$$
\left. - \log q _ {\phi} (z _ {0} \mid x) \right], \quad z _ {1} = \Phi_ {1 \leftarrow 0} ^ {\psi} (z _ {0}). \tag {A.22}
$$

This is the strict training objective of Cola DLM in the setting where the decoder can compute log $p _ { \theta } ( x \mid z _ { 0 } )$ . 
...
$$
\bar {q} _ {\phi} (z _ {0}) := \int q _ {\phi} (z _ {0} \mid x) p _ {\text { data }} (x)   \mathrm{d} x, \tag {A.23}
$$

and the joint distribution 

$$
q (x, z _ {0}) := p _ {\text { data }} (x) q _ {\phi} (z _ {0} \mid x). \tag {A.24}
$$

When the encoder $q _ { \phi }$ and decoder $p _ { \theta }$ are fixed, the dependence of the data-averaged ELBO on ψ appears only through the prior term: 

$$
\mathbb {E} _ {p _ {\text { data }} (x)} \left[ \mathcal {L} _ {\text { ELBO }} (x) \right] = \mathbb {E} _ {q (x, z _ {0})} \left[ \log p _ {\theta} (x \mid z _ {0}) - \log q _ {\phi} (z _ {0} \mid x) \right] + \mathbb {E} _ {q (x, z _ {0})} \left[ \log p _ {\psi} (z _ {0}) \right] \tag {A.25}
$$

$$
= C (\phi , \theta) + \mathbb {E} _ {z _ {0} \sim \bar {q} _ {\phi}} \left[ \log p _ {\psi} (z _ {0}) \right], \tag {A.26}
$$
...
$$
\mathbb {E} _ {z _ {0} \sim \bar {q} _ {\phi}} [ \log p _ {\psi} (z _ {0}) ] = - \mathrm{KL} \big (\bar {q} _ {\phi} (z _ {0}) \| p _ {\psi} (z _ {0}) \big) + \mathbb {E} _ {z _ {0} \sim \bar {q} _ {\phi}} [ \log \bar {q} _ {\phi} (z _ {0}) ],
$$

we obtain 

$$
\max _ {\psi} \mathbb {E} _ {p _ {\text { data }} (x)} \left[ \mathcal {L} _ {\text { ELBO }} (x) \right] \quad \Longleftrightarrow \quad \min _ {\psi} \mathrm{KL} \left(\bar {q} _ {\phi} \left(z _ {0}\right) \| p _ {\psi} \left(z _ {0}\right)\right). \tag {A.27}
$$

Therefore, after fixing the encoder and decoder, the strict prior-learning subproblem is to fit the aggregated posterior $\bar { q } _ { \phi }$ with $p _ { \psi }$ . 
...
$$
\mathbb {E} _ {p _ {\mathrm{data}} (x)} [ \mathcal {L} _ {\mathrm{ELBO}} (x) ] = \mathbb {E} _ {q (x, z _ {0})} [ \log p _ {\theta} (x \mid z _ {0}) ] - I _ {q} (X; Z _ {0}) - \mathrm{KL} \big (\bar {q} _ {\phi} (z _ {0}) \| p _ {\psi} (z _ {0}) \big), \tag {A.28}
$$

where 

$$
I _ {q} (X; Z _ {0}) := \mathbb {E} _ {p _ {\text { data }} (x)} \left[ \mathrm{KL} \left(q _ {\phi} (z _ {0} \mid x) \| \bar {q} _ {\phi} (z _ {0})\right) \right] \tag {A.29}
$$
...
$$
z _ {0} \sim \bar {q} _ {\phi} (z _ {0}), \quad z _ {1} \sim p _ {1} (z _ {1}), \quad t \sim U [ 0, 1 ]. \tag {A.30}
$$
...
Here α can be any sufficiently smooth monotone function; linear interpolation corresponds to $\alpha ( t ) = t$ . The target velocity field of this path is 

$$
u _ {t} (z _ {0}, z _ {1}) := \frac {\mathrm{d} z _ {t}}{\mathrm{d} t} = \dot {\alpha} (t) (z _ {1} - z _ {0}). \tag {A.32}
$$

The Flow Matching loss is then 

$$
\mathcal {L} _ {\mathrm{FM}} (\psi ; \phi) := \mathbb {E} _ {t, z _ {0} \sim \bar {q} _ {\phi}, z _ {1} \sim p _ {1}} \left[ \| \boldsymbol {v} _ {\psi} (z _ {t}, t) - u _ {t} (z _ {0}, z _ {1}) \| _ {2} ^ {2} \right]. \tag {A.33}
$$

For a fixed path construction, the pointwise optimal solution is the conditional expectation 

$$
\boldsymbol {v} _ {\psi} ^ {\star} (z, t) = \mathbb {E} \big [ u _ {t} (z _ {0}, z _ {1}) \mid z _ {t} = z, t \big ]. \tag {A.34}
$$

This is simply a conditional-mean regression result, but it makes the role of Flow Matching precise: under ideal conditions, the learned vector field corresponds to a continuous transport connecting $\bar { q } _ { \phi }$ and $p _ { 1 }$ , which in turn defines the prior $p _ { \psi } ( z _ { 0 } )$ . 
...
$$
\min _ {\psi} \mathcal {L} _ {\mathrm{FM}} (\psi ; \phi), \tag {A.36}
$$
...
$$
L _ {\mathrm{REC}} (x) := - \mathbb {E} _ {q _ {\phi} (z _ {0} | x)} \big [ \log p _ {\theta} (x \mid z _ {0}) \big ], \tag {A.37}
$$

$$
L _ {\text { PRIOR }} (x) := - \mathbb {E} _ {q _ {\phi} (z _ {0} | x)} \left[ \log p _ {\psi} (z _ {0}) \right], \tag {A.38}
$$

$$
L _ {\mathrm{REG}} (x) := \mathbb {E} _ {q _ {\phi} (z _ {0} | x)} \big [ \log q _ {\phi} (z _ {0} \mid x) \big ], \tag {A.39}
$$

so that the total single-sample loss becomes 

$$
L _ {\text { Total }} ^ {\text { strict }} (x) := L _ {\text { REC }} (x) + L _ {\text { PRIOR }} (x) + L _ {\text { REG }} (x) = - \mathcal {L} _ {\text { ELBO }} (x). \tag {A.40}
$$

Taking expectation over the data distribution gives the strict training objective 

$$
\min _ {\phi , \theta , \psi} \mathbb {E} _ {p _ {\text { data }} (x)} \left[ L _ {\text { Total }} ^ {\text { strict }} (x) \right] \quad \Longleftrightarrow \quad \max _ {\phi , \theta , \psi} \mathbb {E} _ {p _ {\text { data }} (x)} \left[ \mathcal {L} _ {\text { ELBO }} (x) \right]. \tag {A.41}
$$
...
$$
\frac {\mathrm{d} z _ {t}}{\mathrm{d} t} = \boldsymbol {v} _ {\psi} (z _ {t}, t), \quad t: 1 \rightarrow 0, \tag {B.2}
$$

so that starting from $z ( 1 ) = z _ { 1 } ~ \mathrm { y i e l d s } ~ z ( 0 ) = z _ { 0 }$ . In practice, Euler, Heun, or higher-order ODE solvers can be used. This step generates global latent semantics rather than tokens themselves. 
...
$$
z ^ {\text { pre }} \sim q _ {\phi} (z ^ {\text { pre }} \mid x ^ {\text { pre }}), \tag {B.4}
$$
...
$$
\hat {z} _ {0} ^ {(b)} = \Phi_ {0 \leftarrow 1} ^ {\psi} \bigl (\epsilon^ {(b)}; z ^ {\mathrm{pre}}, \hat {z} _ {0} ^ {(<   b)} \bigr), \qquad \epsilon^ {(b)} \sim \mathcal {N} (0, I). \tag {B.5}
$$

The response is then decoded as 

$$
\hat {x} ^ {\mathrm{res}} \sim p _ {\theta} \big (x ^ {\mathrm{res}} \mid x ^ {\mathrm{pre}}, z ^ {\mathrm{pre}}, \hat {z} _ {0} ^ {(1: B)} \big). \tag {B.6}
$$
...
where |x| denotes sequence length. Since $\mathcal { L } _ { \mathrm { E L B O } } ( x ) \leq \log p ( x )$ , $\mathrm { P P L } _ { \mathrm { E L B O } }$ should be interpreted as an upper-bound-type estimate of the true perplexity. 
...
$$
z _ {0} ^ {(k)} \sim q _ {\phi} (z _ {0} \mid x), \quad \log w ^ {(k)} = \log p _ {\theta} (x \mid z _ {0} ^ {(k)}) + \log p _ {\psi} (z _ {0} ^ {(k)}) - \log q _ {\phi} (z _ {0} ^ {(k)} \mid x). \tag {B.8}
$$

Then the ELBO-style and IWAE-style estimators are 

$$
\log \widehat {p} _ {\mathrm{ELBO}, K} (x) = \frac {1}{K} \sum_ {k = 1} ^ {K} \log w ^ {(k)}, \quad \log \widehat {p} _ {\mathrm{IWAE}, K} (x) = \log \left(\frac {1}{K} \sum_ {k = 1} ^ {K} e ^ {\log w ^ {(k)}}\right). \tag {B.9}
$$

The IWAE estimator is generally tighter. When IWAE is used for scoring, the corresponding perplexity is 

$$
\mathrm{PPL} _ {\text { IWAE }} := \exp \left(- \frac {\sum_ {x \in \mathcal {D}} \log \widehat {p} _ {\text { IWAE } , K} (x)}{\sum_ {x \in \mathcal {D}} | x |}\right). \tag {B.10}
$$
...
$$
\widehat {\log p _ {\text { cond }}} ^ {\text { ELBO }} (x ^ {\text { res }} \mid x ^ {\text { pre }}) := \mathcal {L} _ {\text { ELBO }} (x ^ {\text { pre }}, x ^ {\text { res }}) - \mathcal {L} _ {\text { ELBO }} (x ^ {\text { pre }}), \tag {B.13}
$$

and 

$$
\widehat {\log p _ {\text {cond} , K}} ^ {\text {IWAE}} (x ^ {\text {res}} \mid x ^ {\text {pre}}) := \log \widehat {p} _ {\text {IWAE}, K} (x ^ {\text {pre}}, x ^ {\text {res}}) - \log \widehat {p} _ {\text {IWAE}, K} (x ^ {\text {pre}}). \tag {B.14}
$$
...
$$
z = (z ^ {(1)}, \dots , z ^ {(B)}, z ^ {(B + 1)}, \dots , z ^ {(B + R)}). \tag {B.15}
$$
...
$$
p (x ^ {\mathrm{res}} \mid x ^ {\mathrm{pre}}, z ^ {\mathrm{pre}}) = \int p _ {\theta} (x ^ {\mathrm{res}} \mid x ^ {\mathrm{pre}}, z ^ {\mathrm{pre}}, z ^ {\mathrm{res}})   p _ {\psi} (z ^ {\mathrm{res}} \mid z ^ {\mathrm{pre}})   \mathrm{d} z ^ {\mathrm{res}}, \tag {B.17}
$$
...
$$
\log p (x ^ {\text { res }} \mid x ^ {\text { pre }}, z ^ {\text { pre }}) \geq E _ {q _ {\phi} (z ^ {\text { res }} | x ^ {\text { pre }}, x ^ {\text { res }}, z ^ {\text { pre }})} \Big [ \log p _ {\theta} (x ^ {\text { res }} \mid x ^ {\text { pre }}, z ^ {\text { pre }}, z ^ {\text { res }}) \tag {B.18}
$$

$$
\left. + \log p _ {\psi} (z ^ {\mathrm{res}} \mid z ^ {\mathrm{pre}}) - \log q _ {\phi} (z ^ {\mathrm{res}} \mid x ^ {\mathrm{pre}}, x ^ {\mathrm{res}}, z ^ {\mathrm{pre}}) \right]. \tag {B.19}
$$

Marginalizing over the true posterior of the prefix latent then yields 

$$
p (x ^ {\mathrm{res}} \mid x ^ {\mathrm{pre}}) = \int p (x ^ {\mathrm{res}} \mid x ^ {\mathrm{pre}}, z ^ {\mathrm{pre}})   p (z ^ {\mathrm{pre}} \mid x ^ {\mathrm{pre}})   \mathrm{d} z ^ {\mathrm{pre}}. \tag {B.20}
$$
...
$$
\mathcal {S} _ {\text { resp }} (x ^ {(B + 1)}; x ^ {(1: B)}, z ^ {(1: B)}) := E _ {q _ {\phi} (z ^ {(B + 1)} | x ^ {(1: B + 1)}, z ^ {(1: B)})} \left[ \log p _ {\theta} (x ^ {(B + 1)} \mid x ^ {(1: B)}, z ^ {(1: B + 1)}) \right. \tag {B.21}
$$

$$
\left. + \log p _ {\psi} (z ^ {(B + 1)} \mid z ^ {(1: B)}) - \log q _ {\phi} (z ^ {(B + 1)} \mid x ^ {(1: B + 1)}, z ^ {(1: B)}) \right]. \tag {B.22}
$$
...
$$
\log p _ {\psi} (z _ {0} ^ {(k)}) = \log p _ {1} (z _ {1} ^ {(k)}) + \ell_ {1} ^ {(k)}. \tag {B.24}
$$

In high dimensions, the divergence can be approximated by the Hutchinson trace estimator: 

$$
\nabla \cdot \boldsymbol {v} _ {\psi} (z _ {t}, t) = \mathrm{Tr} \left(\frac {\partial \boldsymbol {v} _ {\psi} (z _ {t} , t)}{\partial z _ {t}}\right) = \mathbb {E} _ {\boldsymbol {\epsilon}} \left[ \boldsymbol {\epsilon} ^ {\top} \frac {\partial \boldsymbol {v} _ {\psi} (z _ {t} , t)}{\partial z _ {t}} \boldsymbol {\epsilon} \right]. \tag {B.25}
$$
...
Input: sequence $x ;$ encoder $q _ { \phi } ;$ decoder $p _ { \theta } ;$ CNF prior $p _ { \psi } ;$ sample number $K ;$ estimation mode mode ∈ {ELBO, IWAE}. 
...
4. Compute log $p _ { \theta } ( x \mid z _ { 0 } ^ { ( k ) } )$ and log $q _ { \phi } ( z _ { 0 } ^ { ( k ) } \mid x )$ 

5. Construct the importance weight 

$$
\log w ^ {(k)} \leftarrow \log p _ {\theta} (x \mid z _ {0} ^ {(k)}) + \log p _ {\psi} (z _ {0} ^ {(k)}) - \log q _ {\phi} (z _ {0} ^ {(k)} \mid x).
$$
...
Input: prefix $x ^ { \mathrm { p r e } }$ ; response $x ^ { \mathrm { r e s } } ;$ ; sample number $K ;$ estimation mode mode $\in \{ \tt E L B O , I W A E \}$ }. 
...
$$
\widehat {\log p} (x ^ {\mathrm{res}} \mid x ^ {\mathrm{pre}}) \leftarrow \widehat {\log p} (x ^ {\mathrm{full}}) - \widehat {\log p} (x ^ {\mathrm{pre}}).
$$
...
$$
Y = (Y _ {t}) _ {t \in \mathcal {T}}, \quad Y _ {t} \in \mathcal {S}, \tag {C.1}
$$
...
$$
K _ {t \leftarrow s} (\mathrm{d} y _ {t} \mid y _ {s}), \quad s \leq t, \tag {C.2}
$$
...
$$
e _ {\theta} (x \mid y _ {t _ {\star}}), \tag {C.3}
$$

then the text distribution can be written uniformly as 

$$
p _ {\theta} (x) = \int e _ {\theta} (x \mid y _ {t _ {\star}}) p _ {\theta} (\mathrm{d} Y). \tag {C.4}
$$
...
$$
p _ {\eta} ^ {\mathrm{AR}} (x) = \prod_ {i = 1} ^ {L} p _ {\eta} (x _ {i} \mid x _ {<   i}). \tag {C.5}
$$

Its negative log-likelihood is 

$$
- \log p _ {\eta} ^ {\mathrm{AR}} (x) = \sum_ {i = 1} ^ {L} - \log p _ {\eta} (x _ {i} \mid x _ {<   i}). \tag {C.6}
$$
...
$$
\mathbb {P} (S _ {i} = s _ {i} \mid S _ {0: i - 1}) = \mathbb {P} (S _ {i} = s _ {i} \mid S _ {i - 1} = s _ {i - 1}) = p _ {\eta} (x _ {i} \mid x _ {<   i}). \tag {C.8}
$$
...
$$
\mathbb {E} _ {\bar {q} _ {\phi} (z _ {0})} [ \log p _ {b} (z _ {0}) - \log p _ {a} (z _ {0}) ] = \mathrm{KL} \big (\bar {q} _ {\phi} (z _ {0}) \| p _ {a} (z _ {0}) \big) - \mathrm{KL} \big (\bar {q} _ {\phi} (z _ {0}) \| p _ {b} (z _ {0}) \big). \tag {C.12}
$$
...
$$
\mathcal {L} _ {\mathrm{MDM}} (\theta) := \mathbb {E} _ {t \sim U [ 0, 1 ], x _ {0}, x _ {t}} \left[ \frac {1}{t} \sum_ {i = 1} ^ {L} \mathbf {1} [ x _ {t} ^ {i} = M ] (- \log p _ {\theta} (x _ {0} ^ {i} \mid x _ {t})) \right], \tag {C.13}
$$

and satisfies 

$$
- \mathbb {E} _ {p _ {\mathrm{data}} (x _ {0})} \log p _ {\theta} (x _ {0}) \leq \mathcal {L} _ {\mathrm{MDM}} (\theta). \tag {C.14}
$$
...
More precisely, the forward masking process of LLaDA can be embedded into a continuous-time Markov chain. For a single token, let the state space be $\nu \cup \{ M \}$ and define the generator 

$$
G _ {\tau} (a, M) = \lambda (\tau), \qquad G _ {\tau} (a, a) = - \lambda (\tau), \qquad G _ {\tau} (M, M) = 0, \tag {C.15}
$$

where $a \in \nu$ . Then the transition probability from any non-mask state to the absorbing mask state is 

$$
\mathbb {P} (X _ {\tau} = M \mid X _ {0} = a) = 1 - \exp \left(- \int_ {0} ^ {\tau} \lambda (u) \mathrm{d} u\right). \tag {C.16}
$$
...
$$
q (h _ {0} \mid x) = \mathcal {N} (\widetilde {x}, \sigma^ {2} (0) I), \quad q (h _ {t} \mid h _ {s}) = \mathcal {N} \left(h _ {s}, \sigma^ {2} (t) - \sigma^ {2} (s)\right), \tag {C.17}
$$
...
$$
q _ {\phi} (z _ {0} \mid x) = \mathcal {N} (E (x), \sigma_ {0} ^ {2} I), \quad \sigma_ {0} ^ {2} \rightarrow 0, \tag {C.19}
$$

and $E ( x )$ is nearly invertible, then Cola DLM degenerates to the limiting case where $z _ { \mathrm { 0 } }$ is almost just a continuousized representation of the original sequence. This limit clarifies the structural relation: Plaid is closer to the near-observation, non-compressive limit of Cola DLM, while the genuinely new ingredient of Cola DLM is the latent decomposition itself. 
...
$$
z _ {1} \sim p _ {1} (z _ {1}), \quad z _ {0} = \Phi_ {0 \leftarrow 1} ^ {\psi} (z _ {1}), \quad x \sim p _ {\theta} (x \mid z _ {0}). \tag {C.21}
$$
...
$$
\log p (x) \geq \mathbb {E} _ {q _ {\phi} (z _ {0} | x)} \left[ \log p _ {\theta} (x \mid z _ {0}) + \log p _ {\psi} (z _ {0}) - \log q _ {\phi} (z _ {0} \mid x) \right]. \tag {C.22}
$$
...
$$
\mathcal {E} (\mathcal {M}) := \inf _ {p \in \mathcal {M}} \mathrm{KL} \big (p _ {\text { data }} (x) \| p (x) \big). \tag {D.1}
$$
...
$$
\inf _ {p \in \mathcal {M}} \mathbb {E} _ {p _ {\mathrm{data}} (x)} [ - \log p (x) ] = H (p _ {\mathrm{data}}) + \mathcal {E} (\mathcal {M}), \tag {D.2}
$$
...
$$
\mathbb {E} _ {p _ {\mathrm{data}} (x)} [ - \log p _ {\eta} ^ {\mathrm{AR}} (x) ] = H (p _ {\mathrm{data}}) + \mathrm{KL} \big (p _ {\mathrm{data}} \parallel p _ {\eta} ^ {\mathrm{AR}} \big). \tag {D.3}
$$

By contrast, Cola DLM is trained through the ELBO, so for each sample 

$$
- \mathcal {L} _ {\mathrm{ELBO}} (x) = - \log p _ {\theta , \psi} (x) + \mathrm{KL} \big (q _ {\phi} (z _ {0} \mid x) \| p _ {\theta , \psi} (z _ {0} \mid x) \big). \tag {D.4}
$$

Taking expectation over the data distribution gives 

$$
\mathbb {E} _ {p _ {\mathrm{data}} (x)} [ - \mathcal {L} _ {\mathrm{ELBO}} (x) ] = H (p _ {\mathrm{data}}) + \mathrm{KL} \big (p _ {\mathrm{data}} \| p _ {\theta , \psi} \big) + \mathcal {G} _ {\text {Cola DLM}} ^ {\text {infer}} (\phi , \theta , \psi), \tag {D.5}
$$

where 

$$
\mathcal {G} _ {\text { Cola   DLM }} ^ {\text { infer }} (\phi , \theta , \psi) := \mathbb {E} _ {p _ {\text { data }} (x)} \mathrm{KL} \left(q _ {\phi} (z _ {0} \mid x) \| p _ {\theta , \psi} (z _ {0} \mid x)\right) \geq 0 \tag {D.6}
$$
...
population risk $= H ( p _ { \mathrm { d a t a } } )$ + model mismatch + additional gap induced by the training objective. (D.7) 
...
$$
\mathfrak {R} _ {\text { Cola   DLM }} := \mathcal {E} (\mathcal {M} _ {\text { Cola   DLM }}) + \inf _ {\phi , \theta , \psi} \mathcal {G} _ {\text { Cola   DLM }} ^ {\text { infer }} (\phi , \theta , \psi), \tag {D.8}
$$
...
$$
\mathfrak {R} _ {\text { Cola   DLM }} <   \mathfrak {R} _ {\text { AR }} \iff \text { Cola   DLM   is   better   than   AR   at   the   population   level. } \tag {D.9}
$$
...
$$
\mathbb {E} _ {q (x, z _ {0})} \left[ - \log p _ {\theta} (x \mid z _ {0}) \right] \geq H _ {q} (X \mid Z _ {0}). \tag {D.10}
$$

By the mutual-information identity 

$$
H _ {q} (X) = H _ {q} (X \mid Z _ {0}) + I _ {q} (X; Z _ {0}), \tag {D.11}
$$

together with $q ( x ) = p _ { \mathrm { d a t a } } ( x )$ , it follows that 

$$
H _ {q} (X \mid Z _ {0}) = H (p _ {\text { data }}) - I _ {q} (X; Z _ {0}). \tag {D.12}
$$

Therefore, the reconstruction cost satisfies 

$$
\mathbb {E} _ {q (x, z _ {0})} [ - \log p _ {\theta} (x \mid z _ {0}) ] \geq H (p _ {\text { data }}) - I _ {q} (X; Z _ {0}). \tag {D.13}
$$
...
$$
p _ {\text { data }} (x) = \int p ^ {\star} (x \mid g)   p ^ {\star} (g)   \mathrm{d} g, \tag {D.15}
$$
...
$$
\text { learning } p _ {\mathrm{data}} (x) \quad \leadsto \quad \text { learning } p ^ {\star} (g) \text { and } p ^ {\star} (x \mid g). \tag {D.17}
$$
...
Hence, the success or failure of Cola DLM can be reduced to a competition among three curves: the representation rate-distortion curve $\mathcal { G } _ { \mathrm { C o l a ~ D L M } } ^ { \mathrm { i n f e r } }$ . Only when all three favor Cola DLM does latent decomposition become a true theoretical ${ \mathcal { D } } ( R )$ , the prior-approximation curve, and the inference-gap curve advantage. 
...
$$
\log p (x) - \mathcal {L} _ {\mathrm{ELBO}} (x) = \mathrm{KL} \big (q _ {\phi} (z _ {0} \mid x) \| p (z _ {0} \mid x) \big). \tag {D.19}
$$

If the encoder is poor, the model pays a systematic likelihood penalty. The latent bottleneck also introduces the risk of posterior collapse: if the decoder is too strong, the latent dimension is too small, or the regularization is too strong, then $I _ { q } ( X ; Z _ { 0 } )$ may become too small and the encoder may stop transmitting useful information. In addition, accurate evaluation of log $p _ { \psi } ( z _ { 0 } )$ often requires ODE integration and divergence estimation, making evaluation more expensive than in AR models. Importantly, the potential advantage of Cola DLM does not follow automatically from using latent variables and flows. It relies on the inductive assumption that the data contain a favorable low-dimensional semantic latent structure. This requires experiments to verify. 
...
Assumption E.1 (Null hypothesis of purely separable representations). Let d denote the latent dimension, let δ denote the timestep shift, and let $\mathcal { I } _ { d } ( \delta )$ denote the overall performance objective under a fixed semantic metric at latent dimension d. Assume that the following conditions hold: 
...
$$
\mathcal {J} _ {d} (\delta) = \sum_ {i = 1} ^ {d} j _ {i} (\delta) \quad \text { or } \quad \mathcal {J} _ {d} (\delta) = \frac {1}{d} \sum_ {i = 1} ^ {d} j _ {i} (\delta), \tag {E.1}
$$
...
Since $b _ { d }$ is independent of δ and $a _ { d }$ only induces a positive rescaling, it follows that 
...
$$
z _ {t} = \alpha_ {t} \phi (s) + \underbrace {\alpha_ {t} u + \sigma_ {t} \epsilon} _ {\text { total   noise }}. \tag {E.7}
$$
...
$$
I (s; z _ {t}) = \sum_ {i = 1} ^ {d} I (s _ {i}; z _ {t, i}). \tag {E.8}
$$
...
$$
\mathrm{SNR} _ {\text { eff }} (d) \propto d. \tag {E.10}
$$
...
$$
I (g; z _ {t}) = \frac {1}{2} \log \det \left(I + \alpha_ {t} ^ {2} \Sigma_ {g} ^ {1 / 2} A ^ {\top} \left(\sigma_ {\text { eff }} ^ {2} (t) I\right) ^ {- 1} A \Sigma_ {g} ^ {1 / 2}\right). \tag {E.11}
$$

Under a scalar or isotropic approximation, this further reduces to 

$$
I (g; z _ {t}) \approx \frac {r}{2} \log \Bigl (1 + c \cdot d \cdot \mathrm{SNR} _ {\mathrm{eff}} (t) \Bigr), \tag {E.12}
$$
...
$$
\Sigma_ {\text { noise }} (t) = \alpha_ {t} ^ {2} \Sigma_ {u} + \sigma_ {t} ^ {2} I. \tag {E.14}
$$
...
$$
I (s; z _ {t}) = \frac {1}{2} \log \det \left(I + \alpha_ {t} ^ {2} \Sigma_ {s} (\alpha_ {t} ^ {2} \Sigma_ {u} + \sigma_ {t} ^ {2} I) ^ {- 1}\right), \tag {E.15}
$$
...
$$
p (x ^ {\text { res }} \mid c) = \int p _ {\theta} (x ^ {\text { res }} \mid z, c)   p _ {\psi} (z \mid c)   \mathrm{d} z, \tag {F.1}
$$

whereas the local conditional score corresponding to the response-level score in the main text is 

$$
\mathcal {S} _ {\mathrm{resp}} (x) = \mathbb {E} _ {q _ {\phi} (z | x, c)} \Big [ \log p _ {\theta} (x ^ {\mathrm{res}} \mid z, c) + \log p _ {\psi} (z \mid c) - \log q _ {\phi} (z \mid x, c) \Big ]. \tag {F.2}
$$
...
$$
\mathcal {L} _ {\mathrm{FM}} (\psi) = \mathbb {E} \left[ \| v _ {\psi} (Z _ {t}, t, c) - U ^ {\star} \| ^ {2} \right], \tag {F.3}
$$
...
$$
\mathcal {R} (f; z, t, c) := \mathbb {E} \big [ \| f - Y \| ^ {2} \mid Z _ {t} = z, t, c \big ] \tag {F.4}
$$
...
$$
p _ {\psi} (z \mid c) = \sum_ {m = 1} ^ {M} \pi_ {m} (c) \mathcal {N} (\mu_ {m}, \Sigma_ {m}), \quad \sum_ {m = 1} ^ {M} \pi_ {m} (c) = 1, \tag {F.6}
$$

and for a particular sample, its posterior is mainly concentrated on the $m ^ { \star }$ -th mode, namely 

$$
q _ {\phi} (z \mid x, c) \approx \mathcal {N} (\mu_ {m ^ {*}}, \Sigma_ {q}). \tag {F.7}
$$
...
Fix a context $c ,$ let $r ( x ^ { \mathrm { r e s } } ; c )$ denote some external generation-quality function, and let τ be a threshold. Define the decoder-good region by 

$$
A _ {\text { good }} ^ {\tau} (c) := \left\{z: \mathbb {E} _ {x ^ {\text { res }} \sim p _ {\theta} (\cdot | z, c)} \left[ r (x ^ {\text { res }}; c) \right] \geq \tau \right\}, \tag {F.10}
$$
...
$$
A _ {x} ^ {\rho} \subseteq A _ {\mathrm{good}} ^ {\tau} (c) \quad \text { or   at   least } \quad A _ {x} ^ {\rho} \text { is   a   more   local   subregion   inside } A _ {\mathrm{good}} ^ {\tau} (c). \tag {F.12}
$$

This distinction shows that good generation and good conditional PPL are not the same requirement. 

Proposition F.3 (Good generation and poor conditional score can hold simultaneously). Fix a context c. Suppose there exist constants $\alpha , \Delta > 0$ and thresholds τ, ρ such that 

$$
p _ {\psi} \left(A _ {\text { good }} ^ {\tau} (c) \mid c\right) \geq \alpha , \tag {F.13}
$$

$$
q _ {\phi} (A _ {x} ^ {\rho} \mid x, c) \approx 1, \tag {F.14}
$$
...
$$
\log p _ {\theta} (x ^ {\text { res }} \mid z, c) \leq B, \quad \forall z \in A _ {x} ^ {\rho}. \tag {F.16}
$$
...
$$
\mathcal {S} _ {\text { resp }} (x) \leq B - \Delta + o (1), \tag {F.17}
$$
...
$$
\mathcal {S} _ {\mathrm{resp}} (x) = \mathbb {E} _ {q _ {\phi}} \Big [ \log p _ {\theta} (x ^ {\mathrm{res}} \mid z, c) - \big (\log q _ {\phi} (z \mid x, c) - \log p _ {\psi} (z \mid c) \big) \Big ].
$$

By Equation (F.14), the mass of $q _ { \phi }$ outside Aρx is o(1). Combining this with Equations (F.15) and (F.16) yields 

$$
\mathcal {S} _ {\mathrm{resp}} (x) \leq B - \Delta + o (1).
$$
...
$$
\mathcal {S} _ {\text { resp }} (x) = \underbrace {\mathbb {E} _ {q _ {\phi} (z | x , c)} [ \log p _ {\theta} (x ^ {\text { res }} \mid z , c) ]} _ {R (x; c)} - \underbrace {\mathrm{KL} \big (q _ {\phi} (z \mid x , c) \| p _ {\psi} (z \mid c) \big)} _ {G (x; c)}. \tag {F.18}
$$
...
$$
R (x _ {n}; c _ {n}) \to R _ {\mathrm{max}},
$$

while at the same time 

$$
G (x _ {n}; c _ {n}) \rightarrow + \infty \quad \text {   or   remains   significantly   positive,   }
$$
...
$$
\mathcal {S} _ {\mathrm{resp}} (x _ {n}) = R (x _ {n}; c _ {n}) - G (x _ {n}; c _ {n}).
$$
...
$$
\mathrm{KL} (q _ {\phi} \| p _ {\psi}) = \frac {1}{2} \Big [ \mathrm{tr} (\Sigma_ {p} ^ {- 1} \Sigma_ {q}) + (\mu_ {p} - \mu_ {q}) ^ {\top} \Sigma_ {p} ^ {- 1} (\mu_ {p} - \mu_ {q}) - d + \log \frac {\det \Sigma_ {p}}{\det \Sigma_ {q}} \Big ]. \tag {F.20}
$$
...
$$
\mathrm{tr} (\Sigma_ {p} ^ {- 1} \Sigma_ {q}) \quad \mathrm{and} \quad \log \frac {\det \Sigma_ {p}}{\det \Sigma_ {q}}.
$$
...
$$
\text { good   generation } \implies \text { the   prior   hits   a   relatively   large   decoder - good   region }, \tag {F.21}
$$

but 

$$
\text { good   PPL } \implies \text { the   prior   must   additionally   hit   a   much   narrower   gold   posterior   tube. } \tag {F.22}
$$
...
$$
\log p _ {\eta} ^ {\mathrm{AR}} (x ^ {\mathrm{res}} \mid x ^ {\mathrm{pre}}) = \sum_ {i = 1} ^ {N _ {\mathrm{res}}} \log p _ {\eta} (x _ {i} ^ {\mathrm{res}} \mid x ^ {\mathrm{pre}}, x _ {<   i} ^ {\mathrm{res}}), \tag {F.23}
$$

and therefore the PPL is 

$$
\mathrm{PPL} _ {\mathrm{AR}} (x) = \exp \left(- \frac {1}{N _ {\mathrm{res}}} \sum_ {i = 1} ^ {N _ {\mathrm{res}}} \log p _ {\eta} (x _ {i} ^ {\mathrm{res}} \mid x ^ {\mathrm{pre}}, x _ {<   i} ^ {\mathrm{res}})\right). \tag {F.24}
$$
...
$$
\text { training   objective } = \text { the   object   evaluated   by   PPL } = \text { the   object   used   during   generation. } \tag {F.25}
$$
...
$$
\mathcal {L} _ {\mathrm{CE}} = - \log \pi_ {y}. \tag {F.26}
$$
...
$$
p (x ^ {\text { res }} \mid c) = \int \exp \left(\log p _ {\theta} (x ^ {\text { res }} \mid z, c) + \log p _ {\psi} (z \mid c)\right) \mathrm{d} z. \tag {F.27}
$$
...
$$
F _ {x, c} (z) := \log p _ {\theta} (x ^ {\mathrm{res}} \mid z, c) + \log p _ {\psi} (z \mid c)
$$

at its maximizer, then 

$$
\log p (x ^ {\mathrm{res}} \mid c) \approx F _ {x, c} (z _ {x, c} ^ {\star}) + \frac {d}{2} \log (2 \pi) - \frac {1}{2} \log \det H _ {x, c}, \tag {F.28}
$$
...
$$
\mathrm{SNR} (t) = \frac {\alpha_ {t} ^ {2}}{\sigma_ {t} ^ {2}}, \quad \lambda (t) := \log \mathrm{SNR} (t) = \log \frac {\alpha_ {t} ^ {2}}{\sigma_ {t} ^ {2}}. \tag {G.2}
$$
...
$$
\alpha_ {t} ^ {2} = \text { sigmoid } (\lambda (t)), \quad \sigma_ {t} ^ {2} = \text { sigmoid } (- \lambda (t)). \tag {G.4}
$$

Therefore, specifying λ(t) uniquely determines $\left( \alpha _ { t } , \sigma _ { t } \right)$ , and conversely $\left( \alpha _ { t } , \sigma _ { t } \right)$ uniquely determines $\lambda ( t )$ . In this precise sense, 

$$
\boxed {\text { noise   schedule } \Longleftrightarrow \log \mathrm{SNR} \text { curve }} \tag {G.5}
$$
...
$$
\alpha_ {t, \delta} ^ {2} = \operatorname{sigmoid} (\lambda (t) + \delta), \quad \sigma_ {t, \delta} ^ {2} = \operatorname{sigmoid} (- \lambda (t) - \delta). \tag {G.7}
$$
...
$$
\boxed {\text { the   same   raw   timestep } t \text { is   reassigned   to   a   different   logSNR   interval. }} \tag {G.8}
$$
...
To avoid a notation clash with the coefficients $\left( \alpha _ { t } , \sigma _ { t } \right)$ in Equation (G.1), let the interpolation coefficient of the bridge path be denoted by β(t). Consider 
...
$$
\mathcal {L} _ {\mathrm{FM}} = \mathbb {E} _ {t, z _ {0}, z _ {1}} \left[ \| v _ {\psi} (z _ {t}, t) - u _ {t} (z _ {0}, z _ {1}) \| _ {2} ^ {2} \right], \quad t \sim U [ 0, 1 ]. \tag {G.10}
$$

Equivalently, 

$$
\mathcal {L} _ {\mathrm{FM}} = \int_ {0} ^ {1} \ell (t) \mathrm{d} t, \quad \ell (t) := \mathbb {E} _ {z _ {0}, z _ {1}} \left[ \| v _ {\psi} \left(z _ {t}, t\right) - u _ {t} \left(z _ {0}, z _ {1}\right) \| _ {2} ^ {2} \right]. \tag {G.11}
$$

Now let 

$$
\lambda = \Lambda (t) := \log \operatorname{SNR} (t), \tag {G.12}
$$

and assume that Λ is strictly monotone and differentiable, so that the inverse mapping $t = T ( \lambda ) = \Lambda ^ { - 1 } ( \lambda )$ exists. By the change-of-variables formula, 
...
$$
\mathcal {L} _ {\mathrm{FM}} = \int \ell (T (\lambda)) \left| \frac {\mathrm{d} t}{\mathrm{d} \lambda} \right| \mathrm{d} \lambda . \tag {G.14}
$$
...
Therefore, unless Λ(t) is affine, uniform-timestep training is not equivalent to uniform-logSNR training. 

Proof. By Equation (G.14), the pushforward of the uniform measure on t to the λ axis has density 

$$
p _ {\lambda} (\lambda) = p _ {t} (T (\lambda)) \left| \frac {\mathrm{d} t}{\mathrm{d} \lambda} \right|.
$$

Since $p _ { t } ( t ) \equiv 1$ , Equation (G.16) follows immediately. The density $p _ { \lambda } ( \lambda )$ is constant if and only $\{ \textbf { \textit { i f } } \frac { \mathrm { d } \lambda } { \mathrm { d } t }$ is constant, namely if and only if Λ(t) is affine. Hence the two training schemes are not equivalent in general. □ 
...
$$
\mathcal {L} _ {\mathrm{FM}} = \int \mathbb {E} _ {z _ {0}, z _ {1}} \left[ \| v _ {\psi} (z _ {\lambda}, T (\lambda)) - \dot {\lambda} (T (\lambda)) u _ {\lambda} \| _ {2} ^ {2} \right] \left| \frac {\mathrm{d} t}{\mathrm{d} \lambda} \right| \mathrm{d} \lambda . \tag {G.19}
$$
...
$$
\mathcal {L} _ {\mathrm{FM}} = \int w _ {\text { eff }} (\lambda) \mathbb {E} _ {z _ {0}, z _ {1}} \left[ \| \tilde {v} _ {\psi} (z _ {\lambda}, \lambda) - u _ {\lambda} \| _ {2} ^ {2} \right] \mathrm{d} \lambda , \tag {G.21}
$$

where the effective weight is 

$$
w _ {\text { eff }} (\lambda) = \left| \frac {\mathrm{d} \lambda}{\mathrm{d} t} \right|. \tag {G.22}
$$
...
$$
\alpha_ {t} ^ {2} = \mathrm{sigmoid} (\lambda (t)), \qquad \sigma_ {t} ^ {2} = \mathrm{sigmoid} (- \lambda (t)).
$$
...
$$
\Sigma_ {\mathrm{noise}} (t) = \alpha_ {t} ^ {2} \Sigma_ {u} + \sigma_ {t} ^ {2} I, \tag {G.31}
$$

where $\Sigma _ { u }$ is the covariance of the posterior uncertainty term. If Σs denotes the covariance of the semantic signal ϕ(s), then the effective information about s preserved in the latent at time t can be written as 

$$
I (s; z _ {t}) = \frac {1}{2} \log \det \left(I + \alpha_ {t} ^ {2} \Sigma_ {s} (\alpha_ {t} ^ {2} \Sigma_ {u} + \sigma_ {t} ^ {2} I) ^ {- 1}\right). \tag {G.32}
$$
...
$$
\text { the   schedule   effectively   controls   the   curve } t \mapsto I (s; z _ {t}). \tag {G.33}
$$
...
$$
\delta^ {\star} = \arg \max _ {\delta} \operatorname{Perf} \left(I _ {\text {eff}, \delta} (t; d, \Sigma_ {u}, \mathcal {G}, B, \vartheta)\right), \tag {G.34}
$$

where d denotes the latent dimension, $\Sigma _ { u }$ the posterior uncertainty, G the latent geometry or semantic smoothness, B the block size, and ϑ the collection of model parameters. Thus, the problem of selecting the optimal noise schedule is best understood as an effective semantic information calibration problem rather than an isolated numerical tuning problem. 
...
$$
\mathrm{SNR} _ {\text { eff }} (d) \propto d. \tag {G.36}
$$

Consequently, the recoverable information about the shared factor satisfies the approximation 

$$
I (g; z _ {t}) \approx \frac {r}{2} \log \left(1 + c \cdot d \cdot \mathrm{SNR} _ {\text { eff }} (t)\right). \tag {G.37}
$$
...
<table><tr><td>Category</td><td>Setting</td></tr><tr><td>Tokenizer</td><td>OLMo 2 tokenizer</td></tr><tr><td>Supervised data mixing</td><td>None</td></tr><tr><td>Sequence length</td><td>512</td></tr><tr><td>Global batch size</td><td>1,408</td></tr><tr><td>Tokens per step</td><td>720,896</td></tr><tr><td>Precision</td><td>bfloat16</td></tr><tr><td>Optimizer</td><td>AdamW</td></tr><tr><td>Peak learning rate</td><td><eq>1.5 \times 10^{-4}</eq></td></tr><tr><td>Betas</td><td>(0.9, 0.95)</td></tr><tr><td>Weight decay</td><td>0.01</td></tr><tr><td>Gradient clipping</td><td>1.0</td></tr><tr><td>Warmup</td><td>5K steps, linear warmup from <eq>1 \times 10^{-6}</eq> to <eq>1.5 \times 10^{-4}</eq></td></tr><tr><td>LR decay</td><td>Cosine decay to <eq>1 \times 10^{-5}</eq></td></tr><tr><td>Random seed / data seed</td><td>66 / 6198</td></tr><tr><td>Max evaluation new tokens</td><td>32</td></tr></table>
...
$$
q _ {\phi} (z \mid x) = \mathcal {N} \big (\mu_ {\phi} (x), \mathrm{diag} \big (\sigma_ {\phi} ^ {2} (x) \big) \big), \tag {H.1}
$$
...
$$
\log \mathrm{SNR} _ {\text {vae}} = \log \frac {\mathbb {E} _ {x , i} \left[ \mu_ {\phi , i} (x) ^ {2} \right]}{\mathbb {E} _ {x , i} \left[ \sigma_ {\phi , i} (x) ^ {2} \right]}, \tag {H.2}
$$
...
$$
\widehat {\log \mathrm{SNR}} _ {\text {vae}} = \log \frac {\frac {1}{N} \sum_ {n = 1} ^ {N} \frac {1}{D} \sum_ {i = 1} ^ {D} \mu_ {n , i} ^ {2}}{\frac {1}{N} \sum_ {n = 1} ^ {N} \frac {1}{D} \sum_ {i = 1} ^ {D} \sigma_ {n , i} ^ {2}}, \tag {H.3}
$$
...
As shown in Figure 17, we visualize the timestep-sampling distributions induced by different timestep-shift settings. Compared with the uniform schedule, LogitNormal schedules place non-uniform emphasis on different parts of the denoising trajectory: increasing µ shifts the distribution toward later timesteps, while σ controls how concentrated or dispersed the sampling mass is. From the perspective of the main text, this means that timestep shift effectively changes which semantic-information regime is more frequently emphasized during training, rather than simply tuning an isolated numerical hyperparameter. 
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/223bc33a9270d8e834a21ce32e19e37c100daa1cc0d3b29180af01af9692b611.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/d463b7bf47b1ac908f546dd54b7d781139d9ee0bccc1f2c988249644e80b53c8.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/80c9c6239eb7357ea67aface018888860348761b1a790af9eff57380285da317.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/cd8cc4ad561c36ddc5b257fd6716d85f0f452a5fc34ed070b3c8089b88d8e877.jpg)
...
$$
s = \operatorname{sigmoid} (u) = \frac {1}{1 + e ^ {- u}}, \quad s \in (0, 1). \tag {H.6}
$$

The continuous timestep is then defined as 

$$
t = T \cdot s = T \cdot \operatorname{sigmoid} (u), \tag {H.7}
$$
...
$$
s \sim \operatorname{LogitNormal} (\mu , \sigma^ {2}). \tag {H.8}
$$

. Joint DiT x1(BERT loss) Interval Joint DiT x1(BERT loss) 
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/66e0f6b287c85f6be1d265714f32871e50ae7aa430a6e221420b3877d008801f.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/251332b58c552153523b017d7bc5ecc795d614083f3fd3825626d9d4d34ff489.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/7f05e3c7b75b1bd14f05c01c466f8b19c9636fd9dc6d3050c8abe1bb726ccb8d.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/4a8161d50a2d33af016fe72e9e75a0b3b125af7045ea0b3b575848b213004b0f.jpg)



Figure 16 Continuous co-evolution outperforms interval updates with pretrained VAE initialization. Across Task Average, LAMBADA, MMLU, and ${ \mathrm { S I Q A } } ,$ , Joint DiT x1 still shows a stronger overall trend and better final performance than Interval, indicating that even with a stable pretrained VAE initialization, the latent space benefits more from sustained co-evolution with DiT than from periodic freezing.
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/b88703d6ed4c7e2f3f5a5c74cd2186bd4cc5f5cafbe2e777a9a5a0aa1be11f51.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/151a432e4de1e68c0bc046fd18947b7d4170fbe2c8a702c56b12a1816dde916f.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/8cedac3e405c1232997c2fce6164fd52665577cd1cfe7c53a5bdc3e3d402297f.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/073ae2fe04eaaa944278d9dbbd49d550e9c9f2d3953e524df4803d6b1fb3cf80.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/21f4cc2aaae47fac7c12053cbbe61792d0f2a48e60b83202f13eeaa8b9e2db57.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/941266c88508cbf883830c82d5b68c3191da3d88e15513fd68b801eb5bfa2959.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/fc2dfe9362e41acb5a0c66882b702bb64f3969110c9e7860a9366ec79d630143.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/9828a9663060939857d24c4e9c8773f8291a9de46383c9c6eec3c2e84cddacc3.jpg)


![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/40ed1d1bacdbd90fe75dbabb4644221dac65f8cdb51d832e22972bf8676807c0.jpg)
...
where $z _ { K }$ denotes the known region determined by the prefix, and $z _ { U }$ denotes the unknown region to be generated. In an ideal formulation, the task of the first generation block should be understood as 

$$
\text { generate } z _ {U} \text { under   the   fixed   condition } (z _ {\mathrm{pre}}, z _ {K}),
$$

rather than to jointly re-estimate both $z _ { K }$ and $z _ { U }$ . Equivalently, the desired conditional prior is closer to 

$$
p _ {\psi} (z _ {U} \mid z _ {\mathrm{pre}}, z _ {K}),
$$
...
$$
v _ {\mathrm{clean}} ^ {*} (z _ {U, t}, t; z _ {\mathrm{pre}}, z _ {K}) = \mathbb {E} [ u _ {t} ^ {U} \mid z _ {U, t}, t, z _ {\mathrm{pre}}, z _ {K} ].
$$
...
$$
\tilde {c} _ {t} = (z _ {\mathrm{pre}}, \tilde {z} _ {K, t}),
$$

and the corresponding optimal regression target becomes 

$$
v _ {\mathrm{partial}} ^ {*} (z _ {U, t}, t; z _ {\mathrm{pre}}, \tilde {z} _ {K, t}) = \mathbb {E} [ u _ {t} ^ {U} \mid z _ {U, t}, t, z _ {\mathrm{pre}}, \tilde {z} _ {K, t} ].
$$
...
$\mathrm { \ t r a n s p o r t \ t h e \ u n k n o w n \ p a r t \ u n d e r \ f i x e d \ c o n d i t i o n }$ 
...
Why errors accumulate along the flow trajectory. The weakness of partial repaint is further amplified by the continuous nature of flow-based generation. Let the ideal vector field under clean conditioning be $v ^ { \ast } ( z , t ; c )$ , and let the field induced by partial repaint be 
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/5a84fa675193f87e6b2ce2beeed8b321b3a4cff73d154102498e0b79082fa62a.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/bcead1bf81ef64367a75439add6cdf7e277cac688245f9b12f658e65905b6ea3.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/5ff09368c1937e6cd3a2b1a04fe7aff2626de50da09cd52baf1defaaa31f2203.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/c8ce58ff0b12d247aa49e1c6b37da549e84b752f41756e50d0ca67a7d88b6522.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/1d0d52e92d5f04eac30ead890d8c6ab47c53503bdb28741de66d826e20205fdc.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/824b065bbf0b511d94404374de8a75a4ca03c36848cf3ec144dbb7544f729051.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/8d7e1639324e52f58fbf4cea7b80b31887d74e5cff6a60c520a8959f981e7374.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/9c1a71da856a4d9f27d8c51ae40bcafe21af2c304b0794ca1fac40e88ff960a5.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/f658a2c1a006b7ddf513f0ac2c9be9eb229ea249b6189cef0b0d4aa2b28bd96e.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/1f74585c8b1f964c69405d3d9d19acf7f958763869ab3b43362acca174a60f12.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/c0a5b8ea195a221c7dd9448a21e3d7ac33d95d05b952ac88fe79830c63043bb7.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/cb1d6afab39f08c314803f47d8a62a4201fdb59ee3456072d5b8e4eb8925a63c.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/3384412f193d422aec948fc35f64b6d05b1c124ecc8788ef30f6beb83965a002.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/4668467b9a6d94d2a0dd0ae75044d3dac55b22ee2f7aa97e4bd29121d396a8eb.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/e0243f9c7f56e07f91c7c5f24c441f4fe2c51bc668c86785a36cead3e5c70dbf.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/f1feca6754273898a1d68a338908a7abcd0ca27d598bb0d62eadf86fff42fe8b.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/fe7ef2cc400cb24198532346bde17b062b81110daaae9c55196fd3fd73266b70.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/1d7e32b62ac56568051d5e9597403ca4fb8747d83c4c0ac08091017382e00339.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/74a27107744ae78d23029b93b508638e801dfd43ede8a3e65f80a446704f373d.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/54c82dc7938c481fc50645cd917f8622bc8273279a4321a922bd1e9f8b3f41c5.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/0e320eb95c4e5de39bcbb82cc63cb7fe98e97ec2ef570aeada0deef5e74c35ed.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/8d9e75ef6901ae416d8de9fe76db5f22c93f9edeaac357c297702c4659eadec3.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/56263e4101add18ca0c305b410145aae05903b73ca19a3abd724a9d7712e7b2f.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/a243366a3b4922f3c0be9f8e7ebb13615fe0e2aae3650827c89ab9c560beabec.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/2aefa579b7402987e229898d6044c1ac79e02eda246364c2819feee426947ae1.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/e193942afd6093ab0d3152bd5a24bfc7b6f64de7c02693cf53809e3be21fb6a3.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/951e657521e8ad9ae2f0a4f7a9453d8b57ce82d04bbcd3073e6ede6da038db57.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/2113f6424585202aff84959c924044eeda6f1c68456512a1088fd7a9d3fa881f.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/e8f88704763cf5a7afce54b03d3055c0b04825d0525ba3f52be43d4065c5ee09.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/3fa8c342b282c84a71fdc4a3609d029df7b677c74d2e65b14c2fe61e4b0b9ee7.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/9d408acc1b576d86b4eb918708bc3c9b301e47f1da2181cf9c7726ed497fdbd1.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/f3a01a53b1773521ccf40fabf3193658bca05361424b640ea509bc73510086c6.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/23f5f4cb7d8f5c5ea1be52e1c6481213d41007b752ebd5be4fb6396c55123aed.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/f21f992971fcf3ad620d64caaa9bb298490a1d78a9d44e9a8e06e824a645df71.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/286977003936d0bb9d9091b3a2fa6ccb5eb54d4cacbfce0a94c0c66137a5c121.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/7a53ad7861c2be3b195b5d4bdad2f7ffa3022c048e581d6d6064305a952c26cd.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/fbe1dfc07510c7f76a9fa870cecb045df7d772174c37aa0536d800161e7408d9.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/90ce0787c9e8291f9cfc867d7613117e72f3d221ac3d7184b56a4e2524893d0d.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/189061181f67df51de826405ad25276fa65645e01d303c80b5600eceb16f902a.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/8a6674a047de6e3f295ed9929226c9da6d00b7ef7007f1659a746a75926e3759.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/077876afb1fbe74fcd124f674cba50a5786f7e367bea584e27fee4010fc98160.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/e69fe7d8852b3382e844503d8b975ddc5bf135b468bb0ec06323fdf97022225e.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/11eb80039781782df53a3574ff8ee6b9ff6903d787456e9c0c438826ecff3ee2.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/08f33cbcd7d62540d6cdc364288e3b64ae3c87e23f44f2f0b6f7b7187dd03c76.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/a423f512e8ccebe2598733f694d8376821ccf71a4ed575c3524df92bd8afd706.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/53efeb3eb97ebc58ab000a47f8a1ee3a06c1140d275cf72896156dd45cb05d41.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/4748aa0b0b0ad8fac9fbd6d5d48d7523130f6df60e92066aff46110a2a6516c7.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/aedcaeb6af2372d6e4680fcd4f0f7236a63d7f7f02bc9aa468a0923869995078.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/3222b987921964b8d89934141419fc347772953ce6a92be8d2e3c44e7d627cae.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/645e9cbf35344066708e4b8f35307efa14a425c644d77541203554a45563ef17.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/f2d89ccf2b5f2f2826e7a8bd56e40b3ae089646144eb70d2f5ddcba91c3cfaf7.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/8b6725ec02139bd4f3d3269c2e4a07138a30d563d41f97828c37cb7eb067f05c.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/34a684d6e9a1795ce722c9de711b4390350af8d856fe48654c9fc8ec99ccb804.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/4b07560dcd2879e4816191eb09eb2ce23261b3e9a2c0eec0afbc83f02e3f7867.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/226bd02907cc4a563419f039c877406c335b51698c9e0de75dc559bce7a468bf.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/7a6296d72a9d3386e116e3fe0da72d2ef4b95040b0492ee32ab8452e0a5831cb.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/c191121253b13c2f5a05a76befc8f8eae9c769f019a23c48cfac956f0f10b37f.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/4404b2f912fa7917a7210b1f64a0f1ae05cf7e41b5a49db90a5005cb62fd9006.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/e046b54d695ca72fe1a4095a52749c4f8b14b7605d4744f493ac5224e7094860.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/d33ae8e096bca2f8ecea45b17cfe4f343e5e537cf223287653df77fe741c8fea.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/fb0c51a0cd11ae3800f4475c9dd181a6b99c3529989e9c099f9f0e1de5dae5e5.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/d97b0f3a1d22e40157abe0cb4d2bc2b968da2a9c63d67a7dcc54525e1a3757e7.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/78d51876a047252a3f5a04379f5c74eb020f6b9a06e96ab533597f125064b33f.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/a172181cb072d50dfb845145d0f185659910519c4924151634311f0c7d1808cc.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/eb508bcc88552a5bbd4d8efec95036496eb5d6e544b9e061152a4fb0862850c0.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/c580603d249bbf31c05377cf9808bfb774bbf1b87dcf4175f6d36d8a6e6dbcbd.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/76ab4c9a16ffc256414592d73237dfbe6c5d9fb9ff4d7963598710bc7f751e44.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/a3d8b55a7c77792bee430c16fbe29d07a5d3ca89ec4c595a31728f614c557490.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/08f16623e5e11858d823c0037e47b0a8c87b0f48c586122cbbd8d867bf6b4890.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/541a93a7f8e70dd381b2ff0ec9667257e61f8e1ffa2e8a815ebd64cc852e833d.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/875613ebcce7c997476b35ff2aab7ea141be76c572b23560a5c96f003b9b8583.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/37608043d53704a3fedd3f6e35c8cca5b527f24081d6ed660e4843eeabc34c65.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/60a3688955b086db6dd85b6212ace62524510de7e1c18f7240663e795963533b.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/750696afbbd7c92890edc2b67745e8211013186365ecd27b35e6a04862d2c417.jpg)
...
![image](https://cdn-mineru.openxlab.org.cn/result/2026-05-13/b82dd201-5adf-4aba-ac9b-45aa8e293fbf/fea921c1d5b157b95d48b405db1b65bafae0a9d0c0f15b90a58e205de5b9b667.jpg)
